...
# min.prevalence is applied within baseline group only when present, otherwise applied across entire dataset before sorting
make.heatmap <- function(taxa0, map0, min.prevalence=0.25, presence.absence=FALSE, baseline.groups=NULL, outputfn, order.by.num.otus=FALSE, 
        override.rescale=NULL, order.by.prevalence=TRUE)
{
    ...
    if(order.by.prevalence)
    {
        sorted.taxa <- NULL
        if(is.null(baseline.groups)){
            # if baseline group not specified, filter min prevalence across all groups
            prevalences <- apply(taxa0, 2, function(bug.col) mean(bug.col > 0))
            this.taxa.f <- taxa0[,prevalences >= min.prevalence]

            for(i in 1:length(groups))
            {
                this.taxa <- this.taxa.f[map0$Sample.Group==groups[i],]
                ...
                prevalences <- apply(this.taxa, 2, function(bug.col) mean(bug.col > 0))
                # filter prevalence only within the baseline sample group if it is specified 
    ...
    color.list[["Sample.Group"]] <- rep("white", length(unique(map0$Sample.Group)))
    names(color.list[["Sample.Group"]]) <- as.character(unique(map0$Sample.Group))
...
# note: ultimately, samples are sorted by richness and taxa by prevalence
...
# ret <- cluster.otus(otu0=otu0, distfun=distfun)
...
#	hm.colors <- colorRampPalette(c("white","plum"))(27)	# for presence absence only
	hm.colors <- colorRampPalette(c("#f2ece3","#764e80"))(2)	  # presence absence only #005a01
...
#dev.new(width=width, height=height) # for interactive plots only
    ...
    cnames <- shorten.taxonomy(otu.labels)
    # show full otu name
    # cnames <- otu.labels
  ...
  trace = "none", density.info = "none", 
	...
	cexCol=.7, scale="none",
	rowsep=group.end.ix, sepwidth=c(.2,.2), sepcolor="white") # indices for where to draw row separators and width of line

...
# load only when phylogenetic features are available:
    ...
    alpha.metrics <- c("chao1", "observed_otus",  "shannon") #non-phylo  #phylo only : c("PD_whole_tree", "observed_otus", "shannon") 
...
### alpha diversity
    ...
    multiplot.alphadiv.L(map[map$Sub.Study == "L" & map$Subject.ID != "IMP.000",], alphadiv, alpha.metrics, "alphadiv.L.pdf")
...
### PCOA
    ...
    plot.pcoa(map[cs,], dm=wuf_dm, plot.title="Weighted Unifrac", flip.axis=1)
...
### Relative Distance - Longitudinal Plots
    ...
    # remove L.001 -- recruited her way too late and she's throwing off the data
    map_L <- map_L[map_L$Old.Participant.ID != "L.001",]
...
### heatmap
    ...
    make.heatmap(otu, map[cs,], .25, presence.absence=T, baseline.groups=c("Control"), outputfn="heatmap.control.pdf", order.by.num.otus=T)
    make.heatmap(otu, map[cs,], .25, presence.absence=T, baseline.groups=c("HmongThai","KarenThai"), outputfn="heatmap.all.thaibaseline.pdf", order.by.num.otus=T)
    make.heatmap(otu, map[c(hmongthai,hmong_firstgen_cs, hmong_secondgen_cs),], .25, presence.absence=T, baseline.groups=c("HmongThai"), outputfn="heatmap.hmong.thaibaseline.pdf", order.by.num.otus=TRUE)   
    make.heatmap(otu, map[c(hmongthai,hmong_firstgen_cs, hmong_secondgen_cs),], .25, presence.absence=T, baseline.groups=c("Hmong2nd"), outputfn="heatmap.hmong.2ndgenbaseline.pdf", order.by.num.otus=TRUE)   
    make.heatmap(otu, map[c(hmongthai,hmong_firstgen_cs, hmong_secondgen_cs, controls),], .25, presence.absence=T, baseline.groups=c("HmongThai"), outputfn="heatmap.hc.h-baseline.pdf", order.by.num.otus=TRUE)   
    make.heatmap(otu, map[c(hmongthai,hmong_firstgen_cs, hmong_secondgen_cs, controls),], .25, presence.absence=T, baseline.groups=c("Control"), outputfn="heatmap.hc.c-baseline.pdf", order.by.num.otus=TRUE)   
        
    make.heatmap(taxa, map[c(hmongthai,hmong_firstgen_cs, hmong_secondgen_cs),], .25, presence.absence=T, baseline.groups=c("HmongThai"), outputfn="heatmap.taxa.hmong.thaibaseline.pdf", order.by.num.otus=T)   
    make.heatmap(taxa, map[c(hmongthai,hmong_firstgen_cs, hmong_secondgen_cs),], .25, presence.absence=T, baseline.groups=c("Hmong2nd"), outputfn="heatmap.taxa.hmong.2ndgenbaseline.pdf", order.by.num.otus=T)   
    make.heatmap(taxa, map[c(karenthai,karen_firstgen_cs),], .25, presence.absence=T, baseline.groups=c("KarenThai"), outputfn="heatmap.taxa.karen.thaibaseline.pdf", order.by.num.otus=T)   

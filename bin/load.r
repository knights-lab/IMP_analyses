...
food_otu <- read.table(paste(datadir,"../food.otu_L2.txt",sep="/"), sep="\t", quote="", row=1, head=T, comment="", skip=1)
food_alpha <- read.table(paste(datadir,"../food.alpha.txt",sep="/"), sep="\t", quote="", row=1, head=T, comment.char="")

food_otu <- food_otu[,-ncol(food_otu)] # drop taxonomy column
...
map_all$BMI.Class <- factor(map_all$BMI.Class, levels=c("Normal", "Overweight", "Obese")) 
